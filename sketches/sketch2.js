const sketch2 = (p) => {
  ...
  p.setup = function () {
    ...
    finishButton = p.createButton("Finish Growing the Flower");
    finishButton.parent("sketch-container-sk2");
    finishButton.mousePressed(() => {
      finished = true;
      elapsedMinutes = 20; // full bloom
    });
  };
  ...
  p.draw = function () {
    ...
    if (!finished) {
      elapsedMinutes = (p.millis() - startTime) / 60000;
    }
    ...
    p.textSize(18);
    if (finished) {
      p.text("The flower has fully bloomed.", p.width / 2, 650);
    } else {
      p.text("The flower builds over time (1 petal every 5 minutes).", p.width / 2, 650);
